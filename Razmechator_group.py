...
for Group_Key in Regex_dict:
    print(Group_Key)
    Label_df.loc[Label_df[Label_col].str.contains(Regex_dict[Group_Key], flags=re.IGNORECASE), Group_Key] = Group_Key
    Label_df[Group_Key].fillna('', inplace = True)
Label_df
...
#Concatenating
Label_df["Concat"] = Label_df[list(Regex_dict.keys())].apply(lambda x: ','.join(filter(None, x)), axis=1)
Label_df
